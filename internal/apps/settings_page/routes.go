...
func ConfigureSettingsPageGroup(g *echo.Group, deps util.Dependencies) {
	...
	agentAPIGetGroup := g.Group("/set-schedule", util.AgentTokenGetParam(deps.Maigo, maigo.RequestRoleDoctor, maigo.RequestRolePatient))
	...
	g.POST("/edit-schedule/:serial-number", sph.EditSchedulePost, util.AgentTokenGetParam(deps.Maigo, maigo.RequestRoleDoctor, maigo.RequestRolePatient))

	agentAPIFormGroup := g.Group("/pill-dispenser", util.AgentTokenForm(deps.Maigo, maigo.RequestRoleDoctor, maigo.RequestRolePatient))
